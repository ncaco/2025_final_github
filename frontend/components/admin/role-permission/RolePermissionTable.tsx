...
interface RolePermissionTableProps {
  ...
  loading: boolean;
  onViewRolePermission: (rolePermission: RolePermission) => void;
  onDeleteRolePermission: (rolePermission: RolePermission) => void;
  currentPage?: number;
  itemsPerPage?: number;
  totalCount?: number;
}
        ...
        <table className="w-full border-collapse table-fixed">
          ...
          <thead className="bg-background">
            <tr>
              ...
              <th className="h-12 p-1 text-center border font-medium text-muted-foreground">역할명</th>
              <th className="h-12 p-1 text-center border font-medium text-muted-foreground">역할 ID</th>
              <th className="h-12 p-1 text-center border font-medium text-muted-foreground">리소스</th>
              <th className="h-12 p-1 text-center border font-medium text-muted-foreground">액션</th>
              <th className="h-12 p-1 text-center border font-medium text-muted-foreground">활성</th>
      ...
      >
        <table className="w-full border-collapse table-fixed">
          ...
          {rolePermissions.map((rp, index) => {
            ...
            return (
              <tr key={rp.role_permission_id} className="border-b transition-colors hover:bg-muted/50">
                <td className="p-1 border text-center align-middle">{rowNumber}</td>
                <td className="p-1 border text-center align-middle">
                  <div className="font-medium">
                    {rp.role?.role_nm || rp.role_id}
                  </div>
                </td>
                <td className="p-1 border text-center align-middle">
                  <span className="font-mono text-sm">{rp.role_id}</span>
                </td>
                <td className="p-1 border text-center align-middle">
                  <div className="font-medium">
                    {rp.permission?.rsrc || rp.permission_id}
                  </div>
                </td>
                <td className="p-1 border text-center align-middle">
                  <Badge variant="outline" className="text-xs">
                    {rp.permission?.act || rp.permission_id}
                  </Badge>
                </td>
                <td className="p-1 border text-center align-middle">
                  <Badge variant={rp.permission?.actv_yn ? 'default' : 'secondary'} className="text-xs">
                    {rp.permission?.actv_yn ? '활성' : '비활성'}
                  </Badge>
                </td>
                <td className="p-1 border text-center align-middle">
                  {new Date(rp.crt_dt).toLocaleDateString('ko-KR')}
                </td>
                <td className="p-1 border text-center align-middle w-20">
                  <div className="flex justify-center gap-0.5">
                    <Button
                      ...
                      size="icon"
                      className="h-7 w-7"
                      onClick={() => onViewRolePermission(rp)}
                      title="상세 보기"
                    >
                      <svg
                    ...
                    <Button
                      ...
                      onClick={() => onDeleteRolePermission(rp)}
                      title="삭제"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
